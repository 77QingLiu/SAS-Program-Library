...
/*    ODS RTF STARTPAGE=NOW;*/
    PROC REPORT DATA=&TYPE.S.&PGID NOWINDOWS MISSING SPLIT='$'
         style(REPORT)=[PROTECTSPECIALCHARS=OFF JUST=C ASIS=ON OUTPUTWIDTH=99.99%
                        ...
                        %IF %upcase(&FOOTLINE)^=Y %THEN BORDERBOTTOMCOLOR=WHITE;
                        ]
         style(LINES)=[PROTECTSPECIALCHARS=OFF ASIS=ON]
         style(HEADER)=[PROTECTSPECIALCHARS=OFF ASIS=ON JUST=C BORDERBOTTOMCOLOR=WHITE MARGINLEFT=&MARGIN MARGINRIGHT=&MARGIN]
        ...
        COLUMN (( %IF %length(&SUBTITLE1) %THEN "~R/RTF'\fs0\brdrb\brdrs\brdrw10\fs20\b\ql\fi-1152\li1152'%str( )&SUBTITLE1";
                %IF %length(&SUBTITLE) %THEN "~R/RTF%bquote(')\fs0&SUBTLINE\fs20\b\ql\fi-1152\li1152%bquote(')%str( )&SUBTITLE";
                &COLUMN
                ));

        %DO __I=1 %TO &_NDSVAR;
            %LET COLLBL_=%scan(%str( &COLLBL),&__I,%str(|));
            DEFINE %scan(&DISVAR,&__I,%str( )) / DISPLAY FLOW "&COLLBL_" %IF %length(&COLLBL_) %THEN "~R/RTF'\fs0\brdrb\brdrs\brdrw10";
        ...
        %IF %length(&ORDVAR)>0 %THEN %DO;
            ...
            %DO __I=1 %TO &_NODVAR;
                %IF not %index(&CHECKVAR,%scan(&ORDVAR,&__I,%str( ))) & %scan(&ORDVAR,&__I,%str( ))^=&LINE_VAR %THEN %DO;
...
*footnote;
    ...
    %IF %length(&FOOTNOTE)=0 %THEN %DO;
        ODS RTF TEXT="~S={OUTPUTWIDTH=99.99% FONTSIZE=%sysevalf(10*&LINE_FONT)pt} ~R/RTF'\ql'   ";
    %END;
    %ELSE %DO;
        %IF &LINE_VAR=%str( ) %THEN ODS RTF TEXT="~S={OUTPUTWIDTH=99.99% FONTSIZE=%sysevalf(10*&LINE_FONT)pt}       ";;
