...
proc sql;
    create table dupchk as
        select CODELST, case when sum(^missing(CODEVAL)) >1 then "There are duplicate CODEVAL attached to the code list "||cats(CODELST)
                             else ''
                        end as ISSUE length=200
                        from &mlib..cd
                        group by CODELST, DATATYPE, CODEVAL
        union 
