...
Program Location/name: $HeadURL: $
  ...
  Files Created: jjchkacrfpage.log
                 Page Difference Report (aCRF vs Define).xls                       				 
  			 
  Program Purpose: This program is to check aCRF annotation's page number with Define XML.
  
  Page Difference Report (aCRF vs Define).xls contained two sheets (chk_vardef and chk_valdef)
  1. chk_vardef 
...
data vardef2 (keep = annotation freetext_: var);
  ...
  do i = 1 to 20 ;
    ...
    if length(element[i])>8 then put "WAR" "NING:[PXL] variable "   element[i] " length more than 8 char" ;    
        ...
        (substr(VAR,3) ne "TESTCD" and substr(VAR,3) ne "STRESC" and VAR ne "DSDECOD")) then output;                                
    end;                       
  end;
run;
...
data chk_valdef(keep = VALUEOID VALVAL page xml_page);
  retain VALUEOID VALVAL page xml_page; 
  merge xml_valdef valdef ;
  by VALUEOID  VALVAL ;
  if page ne xml_page;
  label page = "aCRF Page";
  label xml_page = "Define Page";    
run;
...
data chk_vardef(keep = DATASET VARIABLE page xml_page);
  retain DATASET VARIABLE page xml_page; 
...
/* Print the output */
%let l_dsets = chk_vardef|chk_valdef; 
%let l_dslabels = VARDEF Page Potential Issue|VALDEF Page Potential Issue;

ods listing close;
    ods Tagsets.ExcelXP
        file="Page Difference Report (aCRF vs Define).xls"
        STYLE=Seaside                                  
...
%let l_dslabel=%scan(&l_dslabels,&i,|);
%do %while ( &l_dset ^= () );  

	ods tagsets.ExcelXP options(sheet_name="&l_dset." absolute_column_width="20");

	title1 justify=left "&l_dslabel";

	PROC REPORT DATA=&l_dset. nowd headline headskip split='|' missing 
	Style(header)=[  cellspacing = 3  borderwidth = 2 bordercolordark = black background=grey foreground=white just=center vjust=center font_size=0.8 font_weight=bold]
