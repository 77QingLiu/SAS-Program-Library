...
options NOTHREADS NOQUOTELENMAX NOSOURCE;
...
filename sdtm_val "&&&outdir..&_tims._&output..xml";

/* Delete current output file if exists */
%if %sysfunc(fexist(sdtm_val)) %then %do;
    data _null_;
        x=fdelete("sdtm_val");
    run;
%end;
...
%if &sysinfo > 64 %then %do;
    ...
    data varatt;
        ...
        if a and b then do;
            if TYPE^=DATATYPE or LENGTH_^=LENGTH or LABEL^=VARLABEL then do;
                CHECKID='Check030000';
                DOMAIN=cats(DATASET);
                DETAILS="The attributes of variables in SDTM datasets is not consistent with the attributes of
 variables in VARDEF, the variable list is "||cats(VARNAME);
            end;
            output;
        end;
...
data _null_;
    ...
    call execute('data temp_cd01; length CHECKID DOMAIN DETAILS $200 pattern $32767.; set out;
                 VAR=prxchange(''s/(\{|\}|\[|\]|\(|\)|\^|\$|\.|\||\*|\+|\?|\/)/\\$1/'', -1, cats('||cats(VARNAME)||'));
...
data _null_;
    ...
    if missing(VARCOM) then do;
        ...
        call execute('data temp_va01; length CHECKID DOMAIN DETAILS $200 PATTERN $32767;
                     set out; VAR=prxchange(''s/(\{|\}|\[|\]|\(|\)|\^|\$|\.|\||\*|\+|\?|\/)/\\$1/'', -1, cats('||cats(VARNAME)||'));
                     PATTERN=''/(,''||cats(VAR)||'',)|^(''||cats(VAR)||'',)|(,''||cats(VAR)||'')$|^(''||cats(VAR)||'')$/'';
...
ods listing close;
ods tagsets.excelxp file=sdtm_val style = XLsansPrinter
