...
-------------------------------------------------------------------------------
  ...
  Files Created:         jjchkacrf.log
                         PXLTimeCode_CRFPageCheck_yyyymmdd.xml
  ...
  Program Purpose:       Check issues below:
                           ...
                           5. To check if the CRF page in study metadata VALDEF is consistent with the page in aCRF
                           6. To check if the CRF page in study metadata VARDEF is consistent with the page in aCRF
...
data temp1;
    ...
    retain CRFPAGE;
    if prxmatch('/^(Page: \d+)/', cats(STRING)) then CRFPAGE=input(scan(STRING, 2, ': '), best.)-1;
