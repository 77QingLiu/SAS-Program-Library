...
-------------------------------------------------------------------------------
  ...
  Program Location/Name: pfesacq_pk_recon_c_lis.sas $HeadURL: $
...
%macro pfesacq_pk_recon_c_lis();

    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_PK_RECON_C_LIS: Start of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

	%if %sysfunc(exist(unblind.pkcdata)) %then %do;
	...
	*** Sort the dataset by Study, Subject and Visit ***;
	...
	proc sort data=unblind.pkcdata out=edata(keep=study siteid subjid visit) nodupkey;
	...
	*** Sort the dataset by Study, Subject ***;
	...
	proc sort data=unblind.pkcdata out=edata nodupkey;
	...
	*** Import the protocol visits document ***;
	...
	proc sort data=unblind.pkcdata out=edata;
		by visit;
	run;
...
/*
	...
	run;
*/
	*** matching date across different visits ***;
	proc sort data=unblind.pk_crf out=crfdata1 nodupkey dupout=temp;
		by study siteid subjid colldate visit;
	run;

	data crfdata1;
		set crfdata1;
		by study siteid subjid colldate visit;
		if first.colldate then count=1;
		else count+1;
		if count>1;
	run;

	*** matching time across different timepoints ***;
	proc sort data=unblind.pk_crf out=crfdata2 nodupkey dupout=temp;
		by study siteid subjid colldate visit testtime pktpt;
	run;

	data crfdata2;
		set crfdata2;
		by study siteid subjid colldate visit testtime pktpt;
		if first.testtime then count=1;
		else count+1;
		if count>1;
	run;
	...
	data lis05;
		length flag $60;
		set crfdata1 crfdata2;

		if visit ne '' and (colldate ne '' or testtime ne '');
	...
	****************************;
	...
	proc sort data=unblind.pkcdata out=edata nodupkey dupout=edata1;
		by acession;
		where acession ne '';
	run;

	proc sort data=unblind.pkcdata out=edata nodupkey dupout=edata2;
		by subjid acession;
		where acession ne '';
	run;
	...
	****************************;
	...
	proc sort data=unblind.pkcdata out=edata nodupkey dupout=edata1;
		by study siteid subjid visit pktpt pksmpid acession;
	run;
...
*/
	...
	*** Concatenate all 12 data sets with lab checks results ***;
	...
	%else %do;
    			%put %str(ERR)OR:[PXL]-------------------------------------------------------------;
    			%put %str(ERR)OR:[PXL] PFESACQ_PK_RECON_C_LIS: alert: Dataset PKCDATA does not exist.;
    			%put %str(ERR)OR:[PXL]-------------------------------------------------------------;
		  %end;
    ...
    %put ;
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_PK_RECON_C_LIS: End of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

%mend pfesacq_pk_recon_c_lis;
