...
-------------------------------------------------------------------------------
  ...
  Program Location/Name: $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/pfe/pfesacq_sae_recon_ae_check5.sas $
 
  Files Created:         None
 
  Program Purpose:       Merge the EDC AE dataset with the safety (CSV/XLS) AE dataset
                         and see if there are any mismatches for the AE start date.                      
...
%macro pfesacq_sae_recon_ae_check5();
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK5: Start of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;


	*** Sort the 2 datasets by Study and Merge them to check if AE start date mismatches exist ***;
	...
	*** EDC ***;
	...
	proc sort data=edc;
		by study subjid dob sex aeser aeterm aedecd2 aecaseid todate aepres aercaus aestdrg deathdt;
	run;
	...
	*** Create a dataset with AE start date mismatches ***;

	data check1;
		length flag $40;
		merge edc(in=a  rename=(fromdate=clinst)) 
			  safe(in=b rename=(fromdate=safest));
		by study subjid dob sex aeser aeterm aedecd2 aecaseid todate aepres aercaus aestdrg deathdt;

		if a and b and clinst ne safest then flag = 'Start Date mismatch';
		if flag ne '';
    ...
    %put ;
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK5: End of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

%mend pfesacq_sae_recon_ae_check5;
