...
-------------------------------------------------------------------------------
  ...
  Creation Date:         18Aug2016 / $LastChangedDate: 2016-12-28 01:08:39 -0500 (Wed, 28 Dec 2016) $

  Program Location/name: $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_JANSS229288_STATS/transfer/qcprog/macros/jjqcblfl.sas $
...
-------------------------------------------------------------------------------
MODIFICATION HISTORY:    Subversion $Rev: 105 $
-----------------------------------------------------------------------------*/
...
%macro jjqcblfl(in_data  =&domain.,
    ...
    /* Add BLFL Flag per group variable */
          %if not (&domain in (LB TR TU FA XC MO))  %then %let GroupVar= STUDYID, USUBJID, &domain.TESTCD; 
    %else %if &domain   =TR %then %let GroupVar=STUDYID, USUBJID, &domain.CAT, &domain.SCAT, &domain.TESTCD, RECORDPOSITION;
    %else %if &domain   =LB %then %let GroupVar=STUDYID, USUBJID, LBCAT, LBSPEC, LBMETHOD, &domain.TESTCD, LBGRPID;
    %else %if &domain   =XC %then %let GroupVar=STUDYID, USUBJID, XCMETHOD, &domain.TESTCD, XCORRES;   
    %else %if &domain   =TU %then %let GroupVar=STUDYID, USUBJID, TUCAT, TUMETHOD, TUTESTCD, RECORDPOSITION;              
    %else %if &domain   =MO %then %let GroupVar=STUDYID, USUBJID, MOTESTCD, MOMETHOD;                  
    %else %let GroupVar =STUDYID, USUBJID, FAOBJ, FATESTCD, datapagename;
    ...
    proc sql;
        create table check as 
            select * from (select *, count(*) as count from base group by &GroupVar %if &ExtraVar ne %then ,&ExtraVar;)
            where count>1
            order by &GroupVar;
    quit;
    data _null_;
