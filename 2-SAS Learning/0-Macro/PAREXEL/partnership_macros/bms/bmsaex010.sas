...
-------------------------------------------------------------------------------
  ...
  Program Location/Name: $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/bms/bmsaex010.sas $

  Files Created:         N/A

  Program Purpose:       Produce the output dataset for Adverse Event tables based on GS_AE_T_X_010
...
%macro bmsaex010(dataOut=, where=, selectColumns=TRT, cutPct=, cutCol=,
                varsBy=, TotalStr="TOTAL SUBJECTS WITH AN EVENT",
                sortSOC=FREQ, sortPT=FREQ, sortCol=TOTAL,
                dataIn=, trtNum=, aebodsys=, aedecod=,  aetoxgrn=,
                SubjidVar=, PopIn=, PopTrtNum=, PopFlag=,
                putLen=4, termColWidth= , resultColWidth=,
                subgroup=, subgroupn=, selectType=E);

  %gmStart( headURL  = $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/bms/bmsaex010.sas $
          , revision = $Rev: 1874 $
    ...
    %let codeFile = %quote(bmsaex010.sas);
         ...
         %bmsaesort(
                     ...
                     SORT_DATAOUT = &ds.1,
                      SORT_BYVARS = &varsby.,
                        ...
                        SORT_TYPE = alph alph alph,
                       SORT_ORDER = asce asce asce,
                      SORT_COLUMN = ,
                      SORT_SUFFIX =
                    );
    %end;
    %else %do;
        ...
        %if &cutPct ne %then %do;
            ...
            %if &cutCol = %then %do;
                %if %upcase(&selectColumns.) = ALL or %upcase(&selectColumns.) = TRT %then %do;
                    %do i = 1 %to &trtCount.;
                        %let cutvarlist = &cutvarlist. _freq_p_&aedecod._&&trt&i.._Total;
                    %end;
                %end;
                %if %upcase(&selectColumns.) = ALL or %upcase(&selectColumns.) = TOTAL %then %do;
                    %let cutvarlist = &cutvarlist. _freq_p_&aedecod._total_Total;
                %end;
            ...
            %else %do;
                ...
                %if &cutColChecked. = 0 %then
                    %gmMessage(codelocation=&codefile.,
                        ...
                        selectType=ABORT);
                %let cutvarlist = _freq_p_&aedecod._&cutCol._Total;
    ...
    %bmsAeFinalize(dataIn= &ds.1, dataOut = &dataout., putLen=&putLen,
                   ...
                   terms = 2, varsby=&varsby., varsbyds=&varsbyds., renameColumnList = &renameColumnList.,
