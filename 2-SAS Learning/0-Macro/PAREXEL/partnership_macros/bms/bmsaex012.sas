...
-------------------------------------------------------------------------------
  ...
  Program Location/Name: $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/bms/bmsaex012.sas $

  Files Created:         N/A

  Program Purpose:       Produce the output dataset for Adverse Event tables based on GS_AE_T_X_012
  ...
  Macro Parameters:
    ...
    Name:                sortCol
      ...
      Default Value:     TOTAL
      Description:       Specifies the column to be used for sorting when sortPT is FREQ.
...
%macro bmsaex012(dataOut=, where=, selectColumns=TRT, cutPct=, cutCol=,
                varsBy=, TotalStr="TOTAL SUBJECTS WITH AN EVENT",
                sortPT=FREQ, sortCol=TOTAL,
                dataIn=, trtNum=, aedecod=,  aetoxgrn=,
                SubjidVar=, PopIn=, PopTrtNum=, PopFlag=,
                putLen=4, termColWidth= , resultColWidth=,
                subgroup=, subgroupn=, selectType=E);

  %gmStart( headURL  = $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/bms/bmsaex012.sas $
          , revision = $Rev: 1874 $
    ...
    %let codeFile = %quote(bmsaex012.sas);
    ...
    %let addTotal = No ;
    %if %upcase(&selectColumns.) = ALL or %upcase(&selectColumns.) = TRT %then %do;
        %let columnNumber = %eval(&trtCount.*3);
        %do i = 1 %to &trtCount.;
            %let columnList = &columnList. &tranPrefix.&&trt&i..grTotal
                                           &tranPrefix.&&trt&i..gr2
                                           &tranPrefix.&&trt&i..gr3;
            %let renameColumnList = &renameColumnList. &tranPrefix.&&trt&i..AnyGrade
                                                       &tranPrefix.&&trt&i..Grade34
                                                       &tranPrefix.&&trt&i..Grade5;

        %end;
    %end;
    %if %upcase(&selectColumns.) = ALL or %upcase(&selectColumns.) = TOTAL %then %do;
        %let columnList = &columnList. &tranPrefix.TotalgrTotal
                                       &tranPrefix.Totalgr2
                                       &tranPrefix.Totalgr3;
        %let renameColumnList = &renameColumnList. &tranPrefix.TotalAnyGrade
                                                   &tranPrefix.TotalGrade34
                                                   &tranPrefix.TotalGrade5;
        %let columnNumber = %eval(&columnNumber + 3);
        %let addTotal = YES;
    %end;
    ...
    data &ds.;
        set &dataIn.;
        _aetotstr = &TotalStr.;
        if &aetoxgrn. in (5) then &varPrefix.grade = 3;
        else if &aetoxgrn. in (3 4) then &varPrefix.grade = 2;
        else &varPrefix.grade = 1;
    run;

    %if &addTotal = YES %then %let tranTotal = &trtNum. &varPrefix.grade;
        %else %let tranTotal =  &varPrefix.grade;

    %* Run bmsfreq macro                                ;;


    %BMSfreq(DataIn=&ds., DataOut=&ds.0, Where=&where., Vars=_aetotstr &aedecod.,
             Count=&SubjidVar &varPrefix.grade,
             Trt=&trtNum., VarsBy=&varsby., Sorttype=ALPH, Sort=,
             keep_num=YES, keep_vars=YES,
             Percent=POP,  PopDS=_bms_popds, trantotal=&tranTotal, popby= &subgroupVar.,
             Putmiss=END,  Missing_word="NOT REPORTED",
             Label="",     FreqPut=&putLen., Shift=0, Transpose=&trtNum. &varPrefix.grade,
             tranPrefix = &tranPrefix. gr);
    ...
    %else %do;
        ...
        %if &cutPct ne %then %do;
            ...
            %if &cutCol = %then %do;
                %if %upcase(&selectColumns.) = ALL or %upcase(&selectColumns.) = TRT %then %do;
                    %do i = 1 %to &trtCount.;
                        %let cutvarlist = &cutvarlist. _freq_p_aedecod_&&trt&i.._Total;
                    %end;
                %end;
                %if %upcase(&selectColumns.) = ALL or %upcase(&selectColumns.) = TOTAL %then %do;
                    %let cutvarlist = &cutvarlist. _freq_p_aedecod_total_Total;
                %end;
    ...
    %bmsAeFinalize(dataIn= &ds.1, dataOut = &dataout., putLen=&putLen,
                   ...
                   terms = 1, varsby=&varsby., varsbyds=&varsbyds., renameColumnList = &renameColumnList.,
