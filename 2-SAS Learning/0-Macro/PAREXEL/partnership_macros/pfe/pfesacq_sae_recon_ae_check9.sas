...
-------------------------------------------------------------------------------
  ...
  Program Location/Name: $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/pfe/pfesacq_sae_recon_ae_check9.sas $
 
  Files Created:         None
 
  Program Purpose:       Merge the EDC AE dataset with the safety (CSV/XLS) AE dataset
                         and see if there are any mismatches for the AE action.                      
...
%macro pfesacq_sae_recon_ae_check9();
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK9: Start of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

	*** Sort the 2 datasets by Study and Merge them to check if AE action mismatches exist ***;
	...
	*** EDC ***;
	...
	data edc;
		...
		by subjid;
		if a;
		aestdrg=upcase(aestdrg);
	run;

	proc sort data=edc;
		by study subjid dob sex aeser aeterm aedecd2 fromdate todate aepres aercaus aecaseid deathdt;
	run;
	...
	*** Create a dataset with AE action mismatches ***;

	data check1;
		length flag $40;
		merge edc(in=a  rename=(aestdrg=clinacn)) 
			  safe(in=b rename=(aestdrg=safeacn));
		by study subjid dob sex aeser aeterm aedecd2 fromdate todate aepres aercaus aecaseid deathdt;

		if a and b and clinacn ne safeacn then flag = 'Action mismatch';
		if flag ne '';
    ...
    %put ;
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK9: End of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

%mend pfesacq_sae_recon_ae_check9;
