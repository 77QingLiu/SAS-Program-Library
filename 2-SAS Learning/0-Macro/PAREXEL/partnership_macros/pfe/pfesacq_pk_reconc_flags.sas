...
-------------------------------------------------------------------------------
  ...
  Macro Output:          PKCRECON dataset is created in the "/unblinded/pfizr&pxl_code./dm/listings/current" Folder
...
%macro pfesacq_pk_reconc_flags();


    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_PK_RECONC_FLAGS: Start of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;


	%if %sysfunc(exist(unblind.pk_crf)) and %sysfunc(exist(unblind.pkcdata)) %then %do;
	...
	*%mu_incremental(labrecon);
	...
	%else %do;
    			%put %str(ERR)OR:[PXL]--------------------------------------------------------------------------;
    			%put %str(ERR)OR:[PXL] PFESACQ_PK_RECONC_FLAGS: alert: Dataset PK_CRF or PKCDATA does not exist.;
    			%put %str(ERR)OR:[PXL]--------------------------------------------------------------------------;
		  %end;
    ...
    %put ;
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_PK_RECONC_FLAGS: End of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

%mend pfesacq_pk_reconc_flags;
