...
%macro pfesacq_sae_recon_dem_check3();
	...
	*** EDC ***;
	...
	proc sort data=edc;
		by study subjid sex aeser aecaseid aeterm aedecd2 fromdate todate aepres aercaus aestdrg deathdt;
	run;
	...
	*** Create a dataset with DOB mismatches ***;


	data check1;
		length flag $40;
		merge edc(in=a  rename=(dob=clindob)) 
			  safe(in=b rename=(dob=safedob));
		by study subjid sex aeser aecaseid aeterm aedecd2 fromdate todate aepres aercaus aestdrg deathdt;

		if a and b and clindob ne safedob then flag = 'DOB mismatch';
		if flag ne '';
