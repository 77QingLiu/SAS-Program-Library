...
-------------------------------------------------------------------------------
  ...
  Program Location/Name: $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/pfe/pfesacq_sae_recon_ae_check4.sas $
 
  Files Created:         None
 
  Program Purpose:       Merge the EDC AE dataset with the safety (CSV/XLS) AE dataset
                         and see if there are any mismatches for the AE preferred term.                      
...
%macro pfesacq_sae_recon_ae_check4();
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK4: Start of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

	*** Sort the 2 datasets by Study and Merge them to check if preferred term mismatches exist ***;
	...
	*** EDC ***;
	...
	proc sort data=edc;
		by study subjid dob sex aeser aecaseid aeterm fromdate todate aepres aercaus aestdrg deathdt;
	run;
	...
	*** Create a dataset with preferred term mismatches ***;

	data check1;
		length flag $40;
		merge edc(in=a  rename=(aedecd2=clindecd)) 
			  safe(in=b rename=(aedecd2=safedecd));
		by study subjid dob sex aeser aecaseid aeterm fromdate todate aepres aercaus aestdrg deathdt;

		if a and b and clindecd ne safedecd then flag = 'Preferred Term mismatch';
		if flag ne '';
    ...
    %put ;
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK4: End of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

%mend pfesacq_sae_recon_ae_check4;
