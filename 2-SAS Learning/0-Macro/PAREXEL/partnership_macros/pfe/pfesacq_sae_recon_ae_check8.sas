...
-------------------------------------------------------------------------------
  ...
  Program Location/Name: $HeadURL: http://kennet.na.pxl.int:7070/svnrepo/LP_BLINDED_GLOBALMACROLIBARY_STATS/partnership_macros/pfe/pfesacq_sae_recon_ae_check8.sas $
 
  Files Created:         None
 
  Program Purpose:       Merge the EDC AE dataset with the safety (CSV/XLS) AE dataset
                         and see if there are any mismatches for the AE causality.                      
...
%macro pfesacq_sae_recon_ae_check8();
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK8: Start of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

	*** Sort the 2 datasets by Study and Merge them to check if AE causality mismatches exist ***;
	...
	*** EDC ***;
	...
	proc sort data=edc;
		by study subjid dob sex aeser aeterm aedecd2 fromdate todate aepres aecaseid aestdrg deathdt;
	run;
	...
	*** Create a dataset with AE causality mismatches ***;

	data check1;
		length flag $40;
		merge edc(in=a  rename=(aercaus=clinrel)) 
			  safe(in=b rename=(aercaus=saferel));
		by study subjid dob sex aeser aeterm aedecd2 fromdate todate aepres aecaseid aestdrg deathdt;

		if a and b and clinrel ne saferel then flag = 'Causality mismatch';
		if flag ne '';
    ...
    %put ;
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_SAE_RECON_AE_CHECK8: End of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

%mend pfesacq_sae_recon_ae_check8;
