...
%macro pfesacq_pk_recona_report();
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_PK_RECONA_REPORT: Start of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;
	...
	%if %sysfunc(exist(unblind.pkarecon&i)) %then %do;

	proc sql noprint;
		select count(*) into:nobs
		from unblind.pkarecon&i;
	quit;

	data _null_;
		set unblind.pkarecon&i;
		where analyte ne '';
		call symput ("analyte",analyte);
	run;

/* Listing outputs */
ODS TAGSETS.EXCELXP
	file="&path_listings./current/&protocol._Analyte %trim(&analyte.) PK Recon_&rundate..xls"
	STYLE=Printer
	OPTIONS ( 
			...
			Autofilter = 'All'
			Frozen_Headers='3'
			Absolute_Column_Width='5, 5, 5, 8, 7, 9, 5, 8, 5, 7,
                                   6, 7, 10, 7, 5, 5, 5, 7, 8, 7, 
                                   7, 7, 7, 7'
  			sheet_name='Analytical PK Reconciliation'
			);
	...
	*title1 justify=left "PK Reconciliation";
	...
	proc sort data=unblind.pkarecon&i out=pkarecon;
	...
	proc report data=pkarecon nowd style(header)=[background=white foreground=black fontweight=bold fontsize=8pt]	
	...
	proc print data=pkarecon LABEL;
		id flag;
	run;
	...
	%else %do;
    			%put %str(ERR)OR:[PXL]--------------------------------------------------------------------;
    			%put %str(ERR)OR:[PXL] PFESACQ_PK_RECONA_REPORT: alert: Dataset PKARECON&i does not exist.;
    			%put %str(ERR)OR:[PXL]--------------------------------------------------------------------;
		  %end;
...
%end;   *** end of do loop ***;
    ...
    %put ;
    %put ---------------------------------------------------------------------;
    %put NOTE:[PXL] PFESACQ_PK_RECONA_REPORT: End of Submacro;
    %put ---------------------------------------------------------------------;
    %put ;

%mend pfesacq_pk_recona_report;
