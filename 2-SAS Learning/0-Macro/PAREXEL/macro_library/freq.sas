%MACRO freq(indsn,var1,var2,by,where,outdsn);
  ...
  %* CLIENT:            PAREXEL                                                  ;
  %*                                                                             ;
  %* PROJECT:           Freq.sas [Macro for producing 2-way (or more)            ;
  %*                              frequencies]                                   ;
  %*                                                                             ;
  %* TIMS CODE:         68372                                                    ;
  %*                                                                             ;
  ...
  %******************************************************************************;
  %*                                                                             ;
  %* PROGRAM NAME:      freq.sas                                                 ;
  %*                                                                             ;
  %* PROGRAM LOCATION:  /opt/pxlcommon/stats/macros/sas/code/freq/ver002         ;
  %*                                                                             ;
  %******************************************************************************;
  %*                                                                             ;
  %* USER REQUIREMENTS: Perform Proc Freq and create output dataset containing   ;
  %*                    specified variables.                                     ;
  %*                                                                             ;
  %* TECHNICAL          The input variables will be used in a Proq Freq to       ;
  ...
  %*                    containing the &BYVARS variables and a frequency count   ;
  %*                    with percentages.                                        ;
  %*                                                                             ;
  %* PROGRAMS CALLED:   N/A                                                      ;
  %*                                                                             ;
  ...
  %*                    Software\SASMacros\Freq\Freq_val.sas                     ;
  %*                                                                             ;
  ...
  %local indsn var1 var2 by where outdsn;

  proc freq data=&indsn noprint;
    tables &var1*&var2 /noprint list outpct out=&outdsn;
    &by;
    &where;
  run;
