%macro cmh(indsn,print,var1,var2,by,byvars,where,weight,mergevar,outdsn);
%******************************************************************************;
%*                          PAREXEL INTERNATIONAL LTD                          ;
%*                                                                             ;
%* CLIENT:            PAREXEL                                                  ;
%*                                                                             ;
%* PROJECT:           Macro to caclulate Cochran-Mantel-Haenszel p-value       ;
%*                                                                             ;
%* TIMS CODE:         68372                                                    ;
%*                                                                             ;
%* SOPS FOLLOWED:     1213                                                     ;
...
%******************************************************************************;
%*                                                                             ;
%* PROGRAM NAME:      cmh.sas                                                  ;
%*                                                                             ;
%* PROGRAM LOCATION:  /opt/pxlcommon/stats/macros/sas/code/cmh/ver002          ;
%*                                                                             ;
%******************************************************************************;
%*                                                                             ;
%* USER REQUIREMENTS: Perform CMH test and create output dataset containing    ;
%*                     specified variables and p-value.  If (due to lack of    ;
%*                     input data) no output dataset is created by PROC FREQ,  ;
%*                     the output dataset should still be created with p-value ;
%*                     set to be missing                                       ;
%*                                                                             ;
%* TECHNICAL          A PROC FREQ will be used to perform the CMH test, and    ;
%* SPECIFICATIONS:     create an output dataset containing the CMH p-value.    ;
%*                     A check will be performed to see whether the output     ;
%*                     dataset exists, and if so the p-value will be formatted.;
%*                     If the output dataset does not exist a blank p-value    ;
...
%*                      VAR1=stratification variable                           ;
%*                      VAR2=input variables (separated by '*')                ;
...
%*                     variable (PVAL) conatining the CMH p-value              ;
%*                                                                             ;
%* PROGRAMS CALLED:   N/A                                                      ;
%*                                                                             ;
%* ASSUMPTIONS/       None                                                     ;
...
%* METHODOLOGY:       CMH_VAL.SAS                                              ;
%*                                                                             ;
%* DEVELOPER:         Simon Gillis                      Date : 1 March 2005    ;
%*                                                                             ;
%* SIGNATURE:         ................................  Date : ............... ;
%*                                                                             ;
%* CODE REVIEWER:     Phil Riley                        Date : 7 March 2005    ;
%*                                                                             ;
%* SIGNATURE:         ................................  Date : ............... ;
%*                                                                             ;
%* USER:              Phil Riley                        Date : 7 March 2005    ;
%*                                                                             ;
%* SIGNATURE:         ................................  Date : ............... ;
%*                                                                             ;
%******************************************************************************;
...
%*                                                                             ;
%* USER:              Dan Higgins                       Date : 13/07/2005      ;
%*                                                                             ;
%* SIGNATURE:         ................................  Date : ............... ;
%*                                                                             ;
%******************************************************************************;
...
proc freq data=&indsn &print;
  tables &var1*&var2 /cmh noprint;
  &by;
  &where;
  &weight;
  output out=&outdsn (keep=&byvars p_cmhrms) cmh;
run;

%if %sysfunc(exist(&OUTDSN)) %then %do;

  data &outdsn (drop=p_cmhrms);
    length pval $6.;
    set &outdsn;
    &mergevar;
    pval=put(round(p_cmhrms,.001),5.3);
  run;
