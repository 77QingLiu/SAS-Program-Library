%macro fishers8(indsn,print,var1,var2,by,byvars,where,weight,mergevar,outdsn);
%******************************************************************************;
%*                          PAREXEL INTERNATIONAL LTD                          ;
%*                                                                             ;
%* CLIENT:            PAREXEL                                                  ;
%*                                                                             ;
%* PROJECT:           Macro to caclulate Fishers exact p-value                 ;
%*                                                                             ;
%* TIMS CODE:         68372                                                    ;
%*                                                                             ;
%* SOPS FOLLOWED:     1213                                                     ;
...
%******************************************************************************;
%*                                                                             ;
%* PROGRAM NAME:      FISHERS8.SAS                                             ;
%*                                                                             ;
%* PROGRAM LOCATION:  /opt/pxlcommon/stats/macros/sas/code/fishers8/ver002     ;
%*                                                                             ;
%******************************************************************************;
%*                                                                             ;
%* USER REQUIREMENTS: Perform Fishers exact test and create output dataset     ;
%*                     containing specified variables and p-value.  If (due to ;
%*                     lack of input data) no output dataset is created by PROC;
%*                     FREQ, the output dataset should still be created with   ;
%*                     p-value set to be missing                               ;
%*                                                                             ;
%* TECHNICAL          A PROC FREQ will be used to perform the Fishers exact    ;
%* SPECIFICATIONS:     test, and create an output dataset containing the       ;
%*                     Fishers exact p-value. A check will be performed to see ;
%*                     whether the output dataset exists, and if so the p-value;
%*                     will be formatted. If the output dataset does not exist ;
%*                     a blank p-value variable will be created.               ;
%*                                                                             ;
...
%*                     variable (PVAL) conatining the Fishers exact p-value    ;
%*                                                                             ;
%* PROGRAMS CALLED:   N/A                                                      ;
%*                                                                             ;
%* ASSUMPTIONS/       None                                                     ;
...
proc freq data=&indsn &print;
  tables &var1*&var2 /exact noprint;
  &by;
  &where;
  &weight;
  output out=&outdsn (keep=&byvars xp2_fish) exact;
run;

%if %sysfunc(exist(&OUTDSN)) %then %do;

  data &outdsn (drop=xp2_fish);
    length pval $6.;
    set &outdsn;
    &mergevar;
    pval=put(round(xp2_fish,.001),5.3);
  run;
