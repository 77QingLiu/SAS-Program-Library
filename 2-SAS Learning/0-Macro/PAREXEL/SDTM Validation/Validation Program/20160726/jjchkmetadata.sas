...
proc sort data=&mlib..cd out=cd;
    by %if stdv=JJ %then CODELST DATATYPE DECOD;
       %else CODELST DATATYPE CODEVAL;;
run;

data cdchk;
    set cd;
    by CODELST DATATYPE CODEVAL DECOD;
    length ISSUE $200;
    if first.DECOD^=last.DECOD then do;
        ISSUE="There are duplicate DECOD attached to the code list "||cats(CODELST);
        output;
    end;
    if first.CODEVAL^=last.CODEVAL then do;
        ISSUE="There are duplicate CODEVAL attached to the code list "||cats(CODELST);
        output;
    end;
    /*
...
*Check issues below:
    ...
    */
    8. To check if the value list of IECAT (JJ standard), CCCAT, DSCAT, FTCAT, QSCAT, 
       LBCAT, LBSPEC or LBMTTHOD is correct
...
data chk&i;
    ...
    %if &i=1 %then %do;
        ...
        if ^missing(CRFPAGE) and
            (prxmatch('/(,\d)/',CRFPAGE) or prxmatch('/(,)$/', cats(CRFPAGE)) or prxmatch("/(')$/", cats(CRFPAGE))) then do;
        ...
        if not prxmatch('/(Derived)/i',ORIGIN) and cmiss(COMMENTS, COMPMETH)^=2 then
            ISSUE="The comment or computer method of "||cats(&var)||" should be missing";
            output;
        end;
        */
        %if &stdv=JANSSEN %then %do;
            %let varl=CCCAT DSCAT FTCAT QSCAT LBCAT LBSPEC LBMTTHOD;
            %do ii=1 %to 7;
                VAR_T="%scan(&varl, &ii)";
                if cats(prxchange('s/(.+)\.(\w+)/\2/', -1, VALUEOID))=VAR_T then do;
                    ...
                    else if VALUELST ^= catx(".", VALUEOID, VALVAL, %if &ii<5 %then substr(VAR_T, 1, 2)||"TESTCD"; 
    ...
    %else %do;
        ...
        if ^missing(CRFPAGE) and prxmatch('/(\,\d)/',CRFPAGE) then do;
        ...
        /*
        if prxmatch('/(Derived)/i',ORIGIN) and cmiss(COMMENTS, COMPMETH)=2 then do;
            ISSUE="The comment or computer method of attached to the variable "||cats(VARNAME)||" of "||cats(&var)||" can not be missing";
            output;
        end;
        if not prxmatch('/(Derived)/i',ORIGIN) and cmiss(COMMENTS, COMPMETH)^=2 then do;
            ISSUE="The comment or computer method of attached to the variable "||cats(VARNAME)||" of "||cats(&var)||" should be missing";
            output;
        end; */
        if prxmatch('/(float)/', DATATYPE) and missing(DECDIG) then do;
...
%macro mdcchk(i=, meta=, var1=, var2=, str=);
proc sql;
...
data final&i(keep=ISSUE);
    ...
    if a and not b then do;
        ISSUE="&str "||cats(&var1)||" in &vardef or &valdef but not in &&&meta";;
        output;
    end;
    if b and not a then do;
        ISSUE="&str "||cats(&var1)||" in &&&meta but not in &vardef or &valdef";;
        output;
    end;
run;
%mend mdcchk;

%mdcchk(i=1, meta=cd, var1=CODELST, var2=CODELST, str=Code list)
%mdcchk(i=2, meta=valdef, var1=VALUELST, var2=VALUEOID, str=Value list)
%mdcchk(i=3, meta=compmeth, var1=COMPMETH, var2=MTHNAM, str=Computational Algorithm Method)
...
%output(ind=valchk, sheet=ValuelistChk)
