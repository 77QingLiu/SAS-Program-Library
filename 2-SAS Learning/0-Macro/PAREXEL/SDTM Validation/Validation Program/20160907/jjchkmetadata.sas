...
filename meta_chk "&&&outdir..&_tims._&output..xml";

/* Delete current output file if exists */
%if %sysfunc(fexist(meta_chk)) %then %do;
    data _null_;
        x=fdelete("meta_chk");
    run;
%end;
...
*Check issues below:
    ...
    6. To check if the CODEVAL is sorted numerically for meaningful RNK CODELST
    7. To Check if the CODEVAL is sorted alphabetically for non-meaningful RNK CODELST
...
/*Meaningful RNK CODELST*/
proc sql noprint;
    create table qmeta as
        select CODELST, &vart, RNK as RNKQ, sum(^missing(CODELST)) as NUMQ, cats(CODELST, &vart) as COMB
        from metastd.cd(where=(CODELST in ('AGEU', 'INFGRISH', 'LGRTYP', 'MHSCAT', 'MODE', 'NCF', 'NRIND', 'NY', 'OUT', 'PATT',
                   'PORTOT', 'POSITION', 'PRSCAT', 'QSREASND', 'RACE', 'REL', 'SEX', 'SIZE', 'SYMPCHG', 'TRTOUT', 'TYPVCON')))
        group by CODELST
        order by CODELST, &vart
        ;

    select distinct COMB into :codeval separated by '", "'
        from qmeta
        ;
quit;
...
data _null_;
    set qmeta;
    call symputx(CODELST, NUMQ);
run;

data cd;
    set meta.cd(where=(CODELST in ('AGEU', 'INFGRISH', 'LGRTYP', 'MHSCAT', 'MODE', 'NCF', 'NRIND', 'NY', 'OUT', 'PATT',
                   'PORTOT', 'POSITION', 'PRSCAT', 'QSREASND', 'RACE', 'REL', 'SEX', 'SIZE', 'SYMPCHG', 'TRTOUT', 'TYPVCON')));
    COMB=cats(CODELST, &vart);
    if COMB in ("&codeval");
run;

proc sql;
    create table mmeta as
        select CODELST, &vart, RNK as RNKM, sum(^missing(CODELST)) as NUMM
        from cd
        group by CODELST
        order by CODELST, &vart
        ;
...
data mmeta01 mmeta02;
    set mmeta;
    if NUMM=input(resolve('&'||cats(CODELST)), best.) then output mmeta01;
    else output mmeta02;
run;

proc sql;
    create table qmeta01 as
        select a.*
        from qmeta a
        inner join
        mmeta b
        on a.CODELST=b.CODELST and a.&vart=b.&vart
		order by a.CODELST, a.RNKQ
        ;
quit;
data qmeta02;
  set qmeta01;
  by CODELST RNKQ;
  if first.CODELST then RNKQ_t = .;
  RNKQ_t + 1;
  if ^missing(RNKQ_t) then RNKQ_ = put(RNKQ_t,z3.);
run;
proc sort; by CODELST &vart; run;

data mean01;
    merge qmeta02 mmeta;
    by CODELST &vart;
    length ISSUE $200;
    if RNKM^=RNKQ_ then ISSUE='The CODEVAL is not sorted numerically for meaningful RNK CODELST '||cats(CODELST);
    if ^missing(ISSUE);
    keep ISSUE;
run;

/*For those in study metadata but not in Janssen metadata*/
data mmeta02;
    set meta.cd(where=(CODELST in ('AGEU', 'INFGRISH', 'LGRTYP', 'MHSCAT', 'MODE', 'NCF', 'NRIND', 'NY', 'OUT', 'PATT',
                   'PORTOT', 'POSITION', 'PRSCAT', 'QSREASND', 'RACE', 'REL', 'SEX', 'SIZE', 'SYMPCHG', 'TRTOUT', 'TYPVCON')));
    COMB=cats(CODELST, &vart);
    if COMB not in ("&codeval");
run;
proc sort; by CODELST &vart; run;

proc sort data=qmeta02; by CODELST RNKQ_t; run;
data qmeta03;
    set qmeta02;
    by CODELST RNKQ_t;
	if last.CODELST;
...
data mean02;
    ...
    RNKN=input(RNK, best.);
    RNK_=lag(RNKN);
    if nmiss(RNK_, RNKN)=0 then RNKL=RNKN - RNK_;
    if first.CODELST and nmiss(RNKN,RNKQ_t)=0 then RNKL=RNKN - RNKQ_t;
    length ISSUE $200;
    if RNKL^=1 then ISSUE='The CODEVAL is not sorted alphabetically for meaningful RNK CODELST '||cats(CODELST);
    if ^missing(ISSUE);
    keep ISSUE;
run;

/*Non-meaningful RNK CODELST*/
proc sql;
     create table mmeta03 as
        select *
        from meta.cd(where=(CODELST ^ in ('AGEU', 'INFGRISH', 'LGRTYP', 'MHSCAT', 'MODE', 'NCF', 'NRIND', 'NY', 'OUT', 'PATT',
                   'PORTOT', 'POSITION', 'PRSCAT', 'QSREASND', 'RACE', 'REL', 'SEX', 'SIZE', 'SYMPCHG', 'TRTOUT', 'TYPVCON')))
        order by CODELST, &vart
        ;
quit;
...
data mean03;
    ...
    RNK_=lag(RNKN);
    if nmiss(RNK_, RNKN)=0 then RNKL=RNKN - RNK_;
    if first.CODELST then RNKL=1;
    length ISSUE $200;
    if RNKL^=1 then ISSUE='The CODEVAL is not sorted alphabetically for non-meaningful RNK CODELST '||cats(CODELST);
    if ^missing(ISSUE);
    keep ISSUE;
run;

data cdchk;
    set cdchk dupchk mean:;
    if ^missing(ISSUE);
    keep ISSUE;
    proc sort nodupkey;
    by  _all_;
...
data final4;
    ...
    if prxmatch('/(ALL)$/', cats(VALVAL)) then VARN=cats(DATASET, "STAT");
...
ods listing close;
ods tagsets.excelxp file=meta_chk style = XLsansPrinter
