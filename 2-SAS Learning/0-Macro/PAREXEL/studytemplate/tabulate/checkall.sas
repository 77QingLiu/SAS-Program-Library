...
data _null_;
    ...
    if lastObs and not (sCtFl and specFl) then do;
        length rc $1;
        rc = resolve(  '%gmMessage(codeLocation=checkall.sas'
                     ||' ,linesOut=%str(Either SDTM CT or spec file is not defined in the metadata)'
...
%gmImportCdiscTerminology( standards = SDTM \ver=&_sdtmCtVer
                           ,dataOut = studyCT 
                         );
