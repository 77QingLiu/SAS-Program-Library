/*-----------------------------------------------------------------------------
  ...
  Sponsor / Protocol No: Janssen / CNTO1959PSA2001
  PXL Study Code: 228456

  SAS Version:  9.3
  Operating System: UNIX
-------------------------------------------------------------------------------
  Author:                  Chase Liu / $LastChangedBy: $
  Creation / modified:     05Apr2016 / $LastChangedDate: $
  ...
  Program Purpose:         To set up page break variable and avoid unnecessary page cut-off

-------------------------------------------------------------------------------
  MODIFICATION HISTORY: Subversion
-----------------------------------------------------------------------------*/

%macro page_optimize(lib=  /* library where data exists */
                      ,PGId  = /* Data set name */
...
*------------------- Final, Merge page data set with source data, and assign page accordingly --------------------;
    ...
    proc sql;
        ...
        where libname="%upcase(&lib)" and memname="%upcase(&PGId)" and not find(name,"grpx_page",'i');
    quit;
    proc sort data=_page_5 out=&lib..&PGId(keep=&_kep_var grpx_page);
        by _group1 _group2 row;
    run;
