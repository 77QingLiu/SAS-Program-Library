...
CODE NAME           : ut_truncate_long_chars
CODE TYPE           : Broad-use Module data transformation
PROJECT NAME        : N/A
...
INFRASTRUCTURE      : SDD, MS Windows, MVS
LIMITED-USE MODULES : N/A
BROAD-USE MODULES   : ut_parmdef ut_logical ut_titlstrt ut_find_long_chars
                      ut_errmsg
...
BROAD-USE MODULE TEMPORARY OBJECT PREFIX: _tc
...
2.0  Gregory Steffens      BMRGCS05Feb2008A Added redefinition of format and
      Teresa Milner         informat variables when their
                            length is changed.
                            Added where clause so as to truncate
                             variable lengths only when the actual
                             length is less than the allocated
                             length.
                            Fixed bug with data sets that contain
                             only one character variable >= length
                             of MAXLENGTH and that variable length
                             MAXLENGTH
**eoh**************************************************************************/
...
%ut_logical(verbose)
%ut_logical(debug)
%local numdsns numdsnsnl dsn_num;
...
data _null_;
  ...
  if first.memname then do;
    memname_num + 1;
    call symput('dsn' || trim(left(put(memname_num,6.0))),
...
%if %bquote(%upcase(&outlib)) ^= %bquote(%upcase(&inlib)) %then %do;
  ...
  *============================================================================;
  ...
  data _null_;
    if eof then do;
      if memname_num > 0 then
       call symput('numdsnsnl',trim(left(put(memname_num,6.0))));
    end;
    set _tclongchars
     %if &numdsns > 0 %then %do;
  ...
  run;
  %if &numdsnsnl > 0 %then %do;
    %do dsn_num = 1 %to &numdsnsnl;
      *------------------------------------------------------------------------;
      %bquote(* &dsn_num of &numdsnsnl Copying data set &&dsn&dsn_num;)
      *------------------------------------------------------------------------;
      data &outlib..&&dsn&dsn_num;
        set &inlib..&&dsn&dsn_num;
      run;
