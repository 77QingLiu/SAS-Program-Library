...
REQUIREMENTS        : https://sddchippewa.sas.com/webdav/lillyce/qa/general/bums/
                    : ut_find_long_chars/documentation/ut_find_long_chars_rd.doc
ASSUMPTIONS         : none
---------------------------------------------------------------------------------
---------------------------------------------------------------------------------
...
BROAD-USE MODULE TEMPORARY OBJECT PREFIX: _fl
...
TYPICAL MACRO CALL AND DESCRIPTION:
  ...
  %ut_find_long_chars(lib=a,maxlength=1)
...
2.0   Gregory Steffens BMRGCS05FEB2008B    Added sorts of _fllongchars and
       Shong Demattos                       _flmaxlen - this was required
                                            in SDD/unix.
                                           Added labels to output data set
                                            variables and proc print.
                                           Added data step in first iteration
                                            of do loop so that proc append
                                            does not generate a warning that
                                            the base data set does not exist

3.0   Chuck Bininger & BMRCB29MAR2011      Update header to reflect code validated
