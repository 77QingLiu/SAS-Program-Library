...
**********************************/
%macro AHGbatchqcva(obj,sasext=sas7bdat,refresh=0,wait=5,version=,prg=qc_va_&obj..sas,sasver=9);
    %local macroname;
    ...
    %if &version eq  %then
        ...
        %AHGrpipe(%bquote(rcsnum -d &root3/data_vai -f &obj..&sasext|sed "s/.*\///" ) ,rptversion)  ;
    ...
    data singleQCprg;
      ...
      obj="&obj";
      prg="&prg";
      lst=tranwrd("qc_va_&obj..sas",'.sas','.lst');
      log=tranwrd("qc_va_&obj..sas",'.sas','.log');
    run;
