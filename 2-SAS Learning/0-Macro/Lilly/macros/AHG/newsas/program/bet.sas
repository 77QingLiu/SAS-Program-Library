...
%macro winone(all,num);
%local money   comb i ;
  %let money=1;
  %let comb=%AHGcombination(&all,&num);
  %do i=1 %to &num;
  %let money=%sysevalf(&money*3.2);
  %end;
  %let money=%sysevalf(&money*&comb);
...
data distr;
  do i=1 to 10000;
  array game game1-game10  way way1-way10 win win1-win10 ;
  array one{1:10} _temporary_(10 45 120 210 252 210 120 45 10 1 );
  array one{1:5} _temporary_(0 102 327 524 335  );
