...
%let awesnap=\\mango\sddext.grp\SDDEXT056\prd\ly2835219\i3y_mc_jpbm\dmc_blinded3;
...
data all;
  ...
  do over allcol;
    ...
    if allcol=:'programs_stat' or allcol=:'replica_programs' then alltype='sas '||alltype;
    if index(allcol,'system_files') then alltype='log lst ';
    if index(allcol,'tfl_output') then alltype='rtf gif sas7bdat';
    if index(allcol,'custom') then alltype='sas7bdat xls*';
    do j=1 to 15;
