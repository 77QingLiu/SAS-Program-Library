%macro chn_ut_status(showall=0,wherestr=1);
...
data tflAll&rdm;
  format Category  name $100.;
  set specs.trackingsheet(rename=(output_name=name ));
  keep name category insheet program_name display_id programmer validator;
  name=lowcase(name);
...
data sdtmlog&rdm;
  ...
  output;
  if not( name=:'relrec' or name=:'supp') then
  do;
  name='supp'||name;
  output;
...
data tfllog&rdm;
  ...
  name=lowcase(scan(scan(line,1,' '),1,'.'));
  LstDate=left(substr(line,index(line,' ')));
  if substr(name,1,1)='_' or name='' OR LstDate='LastWriteTime' then delete;
run;
...
( folder='tfl' and (Filedate<"&adamlatest") )then flag=3;
  ...
  /*    File is not created*/
  if missing(FileDate) or INDEX(FileDate,'-NA-') then flag=9;
...
%AHGalltocharNew(AllFile&rdm,width=500);
...
data tfllabel&rdm;
  ...
  display_id= upcase(DISPLAY_IDENTIFIER);
...
%AHGordvar(AllFile&rdm,id label name category filedate lstdate qcdate comment programmer validator,out=,keepall=0);
