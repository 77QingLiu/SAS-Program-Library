%macro AHGinitbasicAuto(TA=,refreshmeta=0,isam=1)/secure;

     option noxsync mprint;
     %AHGassocfiles;
     %global kanbox autodir;
     %if %AHGblank(&autodir) %then %let autodir=C:\kanbox\My SAS Files;
     %if %AHGblank(&kanbox) %then %let kanbox=C:\kanbox;
  ...
  %let host=gsun81  ;
    ...
    options nodate nonumber nocenter mautosource missing=' '
               sasautos=("&root3/analysis" "&root3/extract" "&root3/macros" "&root2/macros" "&root0/macros" "/Volumes/app/cdars/prod/saseng/pds1_0/macros" "/home/liu04/macros" 

    '!sasroot/sasautos'  )
               fmtsearch=(fmt) cmdmac;
    x "cp -f /home/liu04/stored/current/*.sas7bcat ~/";
    libname netmac ('~/');
