%macro AHGinitbasic(TA=,refreshmeta=0,isam=1)/secure;
%macro dosomething/*copy automac from P drive or skip if no P drive connection */;
  ...
  run;
  %global proot;
	%if %sysfunc(fileexist(&proot\TTE\Ken\Hui\readandwrite\automac\AHG_sas9mac)) %then x "copy &prot\TTE\Ken\Hui\readandwrite\automac\AHG_sas9mac &localroot";
...
%doSomething
  ...
  /*  %else %let bigroot=&pRoot\TTL\Hui;*/
    ...
    %if &theuser=LIUH04		%THEN %LET preadonly=C:\studies\readonly;
