...
;
data _null_;
retain _91 _92 _93 _94 3 ;
set stat_Height end=end;
if length(F)> _91 then _91=length(F);
if length(M)> _92 then _92=length(M);
if length(Name)> _93 then _93=length(Name);
if length(label)> _94 then _94=length(label);
keep _9:;
if end then call symput('len',compbl('' ||put(_91,best.) ||put(_92,best.) ||put(_93,best.) ||put(_94,best.) )) ;
run;
