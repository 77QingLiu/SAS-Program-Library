...
%PUT NOTE: (&SYSMACRONAME) ----------------------------------------------------------;
	...
	%else %if %upcase(&rpt_fmt)=T or %upcase(&rpt_fmt)=L %then %do;
        ...
        %if %eval(&pgnum) eq 1 %then %do;
           %_out2rtf(in = tmpfile, out = rtfout ,_o2r_PrpMdYN=Y ,_o2r_PrpMd=&referenc.);
        %end;
        %else %if %eval(&pgnum) eq 2 %then %do;
           %_out2rtf(in = tmpfile, out = rtfout ,_o2r_PrpPgFMT  = %bquote(Page XXXX of YYYY), 	_o2r_PrpPgFMTC = %bquote(XXXX), _o2r_PrpPgFMTM = %bquote(YYYY),_o2r_PrpMdYN=Y ,_o2r_PrpMd=&referenc.);;
        %end;
        %else %if %eval(&pgnum) eq 3 %then %do;
           %_out2rtf(in = tmpfile, out = rtfout ,_o2r_PrpPgFMT  = %bquote(Page XXXXX of YYYYY), 	_o2r_PrpPgFMTC = %bquote(XXXXX), _o2r_PrpPgFMTM = %bquote(YYYYY),_o2r_PrpMdYN=Y ,_o2r_PrpMd=&referenc.);;
        %end;
...
/*<process sessionid="-6a2db37f:141110e9123:-7c76" sddversion="3.5" cdvoption="N" parseroption="B">*/
/* <parameters>*/
