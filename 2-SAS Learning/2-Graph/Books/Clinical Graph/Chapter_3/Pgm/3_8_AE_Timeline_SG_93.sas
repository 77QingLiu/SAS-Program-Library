...
/*ods html;*/
/*proc print; */
/*  var aeseq  aesev aestdate aeendate aedecod stdate endate stday enday highcap sev;*/
/*run;*/
/*ods html close;*/
...
/*--Concatenate severity into AE2--*/
data ae3;
  set severity ae2_stday;
run;
...
proc print data=attrmap; 
  var id value fillcolor linecolor;
run;
ods html close;

ods listing style=analysis gpath=&gpath image_dpi=&dpi;
ods graphics / reset  width=5.5in height=3in imagename="3_8_1_AE_Timeline_SG_93";
title "Adverse Event Timeline Graph for Patient Id = &pid";
proc sgplot data=ae3 dattrmap=attrmap;
  format stdate date7.;
...
ods graphics / reset  width=5.5in height=3in imagename="3_8_5_AE_Timeline_Grayscale_SG_93";
title "Adverse Event Timeline Graph for Patient Id = &pid";
proc sgplot data=ae3 dattrmap=attrmap_pattern;
  format stdate date7.;
  refline 0 / axis=x lineattrs=(color=black);
