...
ods graphics / reset  width=5in height=3in imagename="3_11_1_1_Dist_of_Max_LFT_SG_V93";
...
proc sgplot data=LFT;
  refline 1 1.5 2 / lineattrs=(pattern=shortdash);
...
ods graphics / reset width=5in height=3in imagename="3_11_2_1_Dist_of_Max_LFT_Group_Journal_SG_V93";
title 'Distribution of Maximum LFT by Treatment';
footnote j=l 'Level of concern is 2.0 for ALAT, ASAT and ALKPH and 1.5 for BILTOT';
proc sgplot data=lft_Grp;
  refline 1 1.5 2 / lineattrs=(pattern=shortdash);
  vbox value / category=test group=drug groupdisplay=cluster 
       lineattrs=(pattern=solid) medianattrs=(pattern=solid) whiskerattrs=(pattern=solid);
  xaxis display=(nolabel);
run;
