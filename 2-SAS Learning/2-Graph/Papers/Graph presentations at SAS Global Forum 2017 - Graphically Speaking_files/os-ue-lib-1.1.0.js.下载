...
function osLogGaUEEvent(event){
	if(osIsValid(gaId)){
	    ga('onespotTracker.send', 'event', 'Onespot User Engagement', event, '', 0, {'nonInteraction': true});
	}else{
...
function osRunItWithJq(osJq) {
    osLog("Onespot tracking script v1.1.0");
	osOnPageActivity();
	if(osIsValid(gaId)){
	...
	if (lastPage) {
		...
		if(decodeURIComponent(lastPage)!=osCurrentUrl.href){
	...
	osSetCookie("os_lastpage_"+osCurrentUrl.hostname,encodeURIComponent(osCurrentUrl.href),0);
}
...
function osGetParameterByName(name) {
	...
	url.href = osCurrentUrl.href;
    var regex = new RegExp("[\\?&]" + name + "=([^&#]*)"),
        results = regex.exec(url.search);
    return results == null ? "" : decodeURIComponent(results[1].replace(/\+/g, " "));
}

function osGetDfaFloodlightParameterByName(name) {
	// This function can be used to extract the origin url form a floodlight tag iframe
    name = name.replace(/[\[]/, "\\[").replace(/[\]]/, "\\]");
    var regex = new RegExp("[;]" + name + "=([^;\\?#]*)"),
